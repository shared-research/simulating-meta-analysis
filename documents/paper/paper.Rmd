...
(ref:res-equal-effects) \color{red} Summary of the simulated equal-effects model. The only estimated parameter is the average effect* $\theta_{f}$ *with the standard error,
95% confidence interval, and the Wald z-test. The z-test evaluates the null hypothesis that the real effect equals zero.\color{black}
...
(ref:res-random-effects) \color{red} Summary of the simulated random-effects model: Compared to the equal-effects model, there are more parameters. The $\beta$ is the average effect ($\theta_{r}$) with the standard error, 95% confidence interval, and the Wald z-test. The $\tau^{2}$ is the estimated heterogeneity and the $I^{2}$ (explained in Section \@ref(re-model)) represents the percentage of total variability due to between-study heterogeneity. \color{black}
